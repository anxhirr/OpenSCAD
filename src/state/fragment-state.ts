...
export function buildUrlForStateParams(state: State) {
  //partialState: {params: State['params'], view: State['view']}) {
  return `${location.protocol}//${location.host}${
    location.pathname
  }#${encodeStateParamsAsFragment(state)}`;
}
...
async function decompressString(compressedInput: string): Promise<string> {
  const compressedData = Uint8Array.from(atob(compressedInput), (c) =>
    c.charCodeAt(0)
  );
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
    ...
    try {
      ...
      return {
        ...
        view: {
          ...
          collapsedCustomizerTabs: validateArray(
            view?.collapsedCustomizerTabs,
            validateString
          ),
