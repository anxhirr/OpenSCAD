...
export default function CustomizerPanel({
  ...
  const groupedParameters = (state.parameterSet?.parameters ?? []).reduce(
    (acc, param) => {
      if (!acc[param.group]) {
        acc[param.group] = [];
      }
  ...
  const setTabOpen = (name: string, open: boolean) => {
    ...
    model.mutate(
      (s) => (s.view.collapsedCustomizerTabs = Array.from(collapsedTabSet))
    );
  };
        ...
        <div
          ...
          <Button
            ...
            style={{
              ...
              visibility:
                value === undefined ||
                JSON.stringify(value) === JSON.stringify(param.initial)
                  ? "hidden"
                  : "visible",
            }}
